...
if (!class_exists('DUP_Pro_EnhancedGoogleMediaFileUpload'))
{
	...
	class DUP_Pro_EnhancedGoogleMediaFileUpload
	{
		...
		/** @var Duplicator_Pro_Google_Client */
		private $client;

		/** @var Duplicator_Pro_Google_Http_Request */
		private $request;
		...
		public function __construct(
		    Duplicator_Pro_Google_Client $client, 
            Duplicator_Pro_Google_Http_Request $request, 
            $mimeType, 
            $data, 
            $resumable = false, 
            $chunkSize = 0,
		...
		public function nextChunk($chunk = false)
		{
			...
			$httpRequest = new Duplicator_Pro_Google_Http_Request(
					$this->resumeUri, 'PUT', $headers, $chunk
			);

			if ($this->client->getClassConfig("Duplicator_Pro_Google_Http_Request", "enable_gzip_for_uploads"))
			{
			...
			{
				return Duplicator_Pro_Google_Http_REST::decodeHttpResponse($response, $this->client);
			}
		}
