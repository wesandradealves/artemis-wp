...
if (!class_exists('DUP_PRO_DropboxClient_UploadInfo'))
{
    ...
    class DUP_PRO_DropboxClient_UploadInfo
    {
        public $upload_id;
        public $next_offset;
...
if (!class_exists('DUP_PRO_DropboxClient'))
{

    class DUP_PRO_DropboxClient
    {
        ...
        function __construct($app_params, $locale = "en", $use_curl = false)
        {
        ...
        public function upload_file_chunk($src_file, $dropbox_path = '', $upload_chunk_size = self::UPLOAD_CHUNK_SIZE, $max_upload_time_in_sec = 15, $offset = 0, $upload_id = null, $server_load_delay = 0)
        {
            ...
            while (($end_of_file == false) && ($time_passed < $max_upload_time_in_sec) & ($error_present == false))
            {
                ...
                // NOTE: it returns 400 if dropbox already has that chunk
                ...
                {
                    $error_message = "Response to {$url} is missing and file content is $file_contents.  Make sure 'extension=php_openssl.dll' is in the php.ini for windows.  ";
                    $error_message .= 'On Linux based systems check for the --with-openssl[=DIR] flag.';
            ...
            if ($file_size > self::MAX_UPLOAD_CHUNK_SIZE)
            {
                ...
                while (!feof($fh))
                {
                    ...
                    if (empty($upload_id))
                    {
                        $upload_id = $response->upload_id;
                        if (empty($upload_id))
                            throw new DropboxException("Upload ID empty!");
                    }
                }
        ...
        private function createRequestContext($url, $method, &$content = null, $oauth_token = -1)
        {
            ...
            $oauth = new DUP_PRO_OAuthSimple($this->consumerToken['t'], $this->consumerToken['s']);
