...
class DashboardWidget
{
    ...
    protected static function renderSections()
    {
        ...
        if (($schedules = DUP_PRO_Schedule_Entity::getIds()) === false) {
            $schedules = [];
        }
        ...
        if (
            ($recoverId = DUP_PRO_Package_Recover::getRecoverPackageId()) !== false &&
            ($recoverPackage = DUP_PRO_Package::get_by_id($recoverId)) !== false
        ) {
            $recoverTime       = (int) strtotime($recoverPackage->Created);
    ...
    public static function getLastBackupString()
    {
        ...
        $createdTime = date(get_option('date_format'), (int) strtotime($lastPackage[0]->Created));
