...
class CapMng
{
    ...
    */
    ...
    private function save()
    {
        ...
        if ($updateRoles && update_option($roles->role_key, $roles->roles) == false) {
            DUP_PRO_Log::trace('Capabilites save failed option ' . $roles->role_key . ' roles: ' . print_r($roles->roles, true));
            return false;
        }
        if (update_option(self::OPTION_KEY, $this->capabilities) == false) {
            DUP_PRO_Log::trace('Capabilites save failed option ' . self::OPTION_KEY . ' capabilities: ' . print_r($this->capabilities, true));
            return false;
        }
    ...
    public static function getCapsInfo()
    {
        return [
            ...
            self::CAP_STORAGE        => [
                ...
                'desc'   => __(
                    'The capability to create and modify storage. Those with the "Package Create" capability can select existing storage but cannot edit it',
                    'duplicator-pro'
                ),
            ],
            self::CAP_BACKUP_RESTORE => [
