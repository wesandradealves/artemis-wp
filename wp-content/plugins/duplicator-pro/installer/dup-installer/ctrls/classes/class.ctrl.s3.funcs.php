...
LONGMSG;
    ...
    public function generalCleanup()
    {
        ...
        if ($paramsManager->getValue(PrmMng::PARAM_EMPTY_SCHEDULE_STORAGE)) {
            ...
            DUPX_DB::mysqli_query(
                $this->dbh,
                "DELETE FROM `" . $entitiesTable . "` WHERE `type` = 'DUP_PRO_Storage_Entity' AND `id` != " . $archiveConfig->defaultStorageId
            );
            Log::info(" - REMOVED " . mysqli_affected_rows($this->dbh) . " storage items");

            DUPX_DB::mysqli_query(
                $this->dbh,
                "DELETE FROM `" . $entitiesTable . "` WHERE `type` = 'DUP_PRO_Schedule_Entity'"
            );
