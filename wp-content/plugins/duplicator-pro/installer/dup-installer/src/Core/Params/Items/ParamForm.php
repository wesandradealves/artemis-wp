...
class ParamForm extends ParamItem
{
     ...
     * @param array<string, mixed> $attr     param attributes
     * @param array<string, mixed> $formAttr form attributes
     *
     * @return self
     */
     ...
     *
     * @param array{type?: string, label?: ?string, id?: ?string, btnAction?: ?string, attrs?: array<string, mixed>} $data postix element data
     *
     * @return array{type: string, label: ?string, id: ?string, btnAction: ?string, attrs: array<string, mixed>}
     */
    protected function prefixPostfixElem(array $data)
    {
        $default = array(
            'type'      => 'none',
            'label'     => null,
     ...
     *
     * @param array{type: string, label: ?string, id: ?string, btnAction: ?string, attrs: array<string, mixed>} $element element data
    ...
    protected static function renderSelectOptions($options, $inputValue)
    {
        ...
        foreach (self::getGroupedOptions($options) as $option) {
            if ($option->isHidden()) {
                continue;
            }
    ...
    /**
     * Group the options so that all those belonging to the same group are sequential.
     * Maintains the order of options within the same group
     *
     * @param ParamOption[] $options options list
     *
     * @return ParamOption[]
     */
    protected static function getGroupedOptions($options)
    {
        $groups = [];
        foreach ($options as $opt) {
            if (in_array($opt->getOptGroup(), $groups)) {
                continue;
            }
            $groups[] = $opt->getOptGroup();
        }
    ...
    protected static function getDefaultAttrForFormType($formType)
    {
        $attrs = array(
            ...
            'labelClasses'          => array(
                'label',
                'main-label',
            ), // label classes (the label html is <span class="classes" >label</span>
        ...
        switch ($formType) {
            ...
            case self::FORM_TYPE_BGROUP:
                ...
                $attrs['inputContainerClasses'] = array(
                    'input-container',
                    'btn-group',
                );
                break;
