...
use DuplicatorPro\Guzzle\Http\Message\EntityEnclosingRequest;
use DuplicatorPro\Guzzle\Http\Message\RequestInterface;
use DuplicatorPro\Guzzle\Parser\ParserRegistry;
use DuplicatorPro\Guzzle\Http\Url;
...
class CurlHandle
{
    ...
    public static function factory(RequestInterface $request)
    {
        ...
        if ($requestCurlOptions->get('progress')) {
            ...
            $curlOptions[CURLOPT_PROGRESSFUNCTION] = function () use ($mediator, $handle) {
                ...
                if (\DUP_PRO_U::isCurlResourceOrInstance($args[0])) {
                    array_shift($args);
                }
    ...
    {
        if (!\DUP_PRO_U::isCurlResourceOrInstance($handle)) {
    ...
    {
        if (\DUP_PRO_U::isCurlResourceOrInstance($this->handle)) {
    ...
    {
        return \DUP_PRO_U::isCurlResourceOrInstance($this->handle);
    }
